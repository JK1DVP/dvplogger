...
#define BAND_MASK_SHF  0b0001111000000000
#define BAND_MASK_HF   0b0000000000111111
#define BAND_MASK_5600 0b0000100000000000
#define BAND_MASK_2400 0b0000010000000000
#define BAND_MASK_1200 0b0000001000000000
#define BAND_MASK_430  0b0000000100000000
#define BAND_MASK_144  0b0000000010000000
#define BAND_MASK_50   0b0000000001000000
//                       2111521415221731
//                       4800642340814 58
//                          G
...
struct rig {
  char name[LEN_RIG_NAME]; // name of the rig
...
struct radio {
  ...
  char msg_reserved[100];  // if not \0, message to be sent in 2BSIQ response is saved msg_reserved[]; 
  ...
  int antenna; // antenna id 0-9 connected to the radio selected based on struct antenna information; -1 not connected
...
struct antenna {
  char *name[10];
  char id[N_BAND][3]; // operative antenna ids 0-9 for each band -1 not defined
} ;
...
struct logwindow {
  ...
  // voice player
  bool f_playing=0;
  char playing[50]; // playing voice 
  
  // repeat function key operation 関係
  //  int repeat_func_key ; // function key pressed when repeat function initiated 
  //  int repeat_func_radio; // memory which radio when started repeat function  --> repeat_func_radio is the message sending tx 
...
// phone 等の場合、送信終了しているか確認したらrepeat_func_timerをセットするようにできるかな。
 ...
 // char f_rotator_target_az_ready; // 1 to signal to send target azimuth (not yet used)
  ...
  char sent_exch[LEN_SENT_EXCH_WINDOW + 3];
  ...
  char cwbuf_display[LEN_CW_SEND_BUF+1];


  
  // contest type related

  int f_off_contest; // current QSO does not dupecheck,(multi check), and no TU seq.  and record with C:{f_off_contest} default -1: on the contest (tentatively 25/5/3)  make dupe does not count the qso with C: marks
  ...
  int seqnr_band[N_BAND]; // number of QSOs for the band 
  int txnum;
...
//    char remarks[40];
  ...
  uint8_t all[QSO_RECORD_SIZE];
  uint8_t all1[QSO_RECORD_SIZE+1]; // pad for tailing '\0'
};
...
struct bandmap_disp {
  ...
  int cursor[N_BAND]; // number of cursor colum in the display (not including on-freq display) (edit will be performed on the column
  int top_column[N_BAND]; // display offset column to start display from this column (scroll like appearance)
