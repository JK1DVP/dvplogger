...
static void (*s_loader_boot_init_func)(void)=NULL;
static void (*s_loader_reset_init_func)(void)=NULL;
static void (*s_loader_port_reset_target_func)(void)=NULL;
static void (*s_loader_port_enter_bootloader_func)(void)=NULL;
...
esp_loader_error_t loader_port_esp32_init(const loader_esp32_config_t *config)
{
    ...
    s_loader_boot_init_func=config->loader_boot_init_func;
    s_loader_reset_init_func=config->loader_reset_init_func;
    s_loader_port_reset_target_func=config->loader_port_reset_target_func;
    s_loader_port_enter_bootloader_func=config->loader_port_enter_bootloader_func;
    ...
    //    if (s_reset_trigger_pin>=100) {
    //      mcp.digitalWrite(s_reset_trigger_pin-100,0);
    //      mcp.pinMode(s_reset_trigger_pin-100,OUTPUT);
    //    } else {
    if (s_loader_reset_init_func!=NULL) {
      s_loader_reset_init_func();
    } else {
      ...
      //    if (s_gpio0_trigger_pin>=100) {
      //      mcp.digitalWrite(s_gpio0_trigger_pin-100,0);
      //      mcp.pinMode(s_gpio0_trigger_pin-100,OUTPUT);
      //    } else {
    if (s_loader_boot_init_func!=NULL) {
      s_loader_boot_init_func();
    } else {
...
{
  //    if (s_gpio0_trigger_pin>=100) {
  //      mcp.digitalWrite(s_gpio0_trigger_pin-100, SERIAL_FLASHER_BOOT_INVERT ? 1 : 0);
  //    } else {
  if (s_loader_port_enter_bootloader_func!=NULL) {
    s_loader_port_enter_bootloader_func();
  } else {
    ...
    //    if (s_gpio0_trigger_pin>=100) {
    //      mcp.digitalWrite(s_gpio0_trigger_pin-100, SERIAL_FLASHER_BOOT_INVERT ? 0 : 1);
    //    } else {
...
{
  if (s_loader_port_reset_target_func!=NULL) {
    s_loader_port_reset_target_func();
  } else {
