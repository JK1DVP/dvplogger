/**
 * ch9350if.cpp
 * Main module of CH9350L keyboard interface library for Arduino.
...
// composite deviceの場合、rep[0]にreport_idが来るので1バイトずらす。
//
...
void ch9350if::ch9350_loop() {
  ...
  /*  unsigned long tmp;
      tmp=millis();	    
      if ((ch_rx_state != has_none) && tmp> timeout) {
      ch_rx_state=has_none;
      }
      timeout=tmp+100; // 100 ms timeout
  */
...
void ch9350if::ch9350_setup() {
  ...
  _init();		
  resume();
  CH_SERIAL.write(state1_frame,sizeof(state1_frame));
  //  Serial.println("stete1 frame sent");
  delay(100);
}
